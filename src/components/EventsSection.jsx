      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="space-y-6">
          {items.map((event, index) => (
            <article
              key={index}
              className="flex flex-col sm:flex-row gap-6 p-6 rounded-2xl border border-teal-mid/30 bg-carbon/60 hover:border-crimson/40 transition-colors"
            >
              ...
              <div className="flex-1 border-t sm:border-t-0 sm:border-l border-teal-mid/30 pt-4 sm:pt-0 sm:pl-6">
                <h3 className="font-display text-xl text-ivory mb-2">{event.title}</h3>
                <p className="font-serif text-sand mb-4">{event.description}</p>
                <a
                  href={event.ctaUrl}
